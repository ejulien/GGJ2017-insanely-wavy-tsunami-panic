...
void spawn_homes(core::Scene *scn)
{
	...
	for (uint i = 0; i < nodes.size(); ++i)
		if (nodes[i]->GetName().starts_with("maison"))
			++home_count;

	log(stringify("home count: %1").arg(home_count));
	homes.reserve(home_count);
...
void particle_sample_ground(const Vector3 &pos, Vector3 &n, float &h)
{
	...
	float *p_f = reinterpret_cast<float *>(heightmap.c_ptr());
...
//
typedef ArraySort<bytesort_float, int> ParticleSort;

array<ParticleSort::entry> sort_a, sort_b;
...
void update_particle_field()
{
	...
	// SAP?
	sort_a.reserve(count);
	sort_b.reserve(count);

	for (uint i = 0; i < count; ++i) {
		sort_a[i].v = to_bytesort_float(particles[i].pos.AXIS_ACCEL);
		sort_a[i].o = i;
	}

	auto sorted_particles = ParticleSort::bytesort(count, &sort_a, &sort_b);
	...
	for (int i = 0; i < count; ++i) {
		auto p_a_i = (*sorted_particles)[i].o; // 1st particle index
		auto &p_a = particles[p_a_i];

		// determine range start
		int j = i;
		for (; j > 0; --j)
			if (particles[(*sorted_particles)[j].o].pos.AXIS_ACCEL < p_a.pos.AXIS_ACCEL - cohesion_limit)
		...
		for (; j < count; ++j) {
			...
			auto p_b_i = (*sorted_particles)[j].o; // 2nd particle index
			auto &p_b = particles[p_b_i];
...
shared_ptr<core::IsoSurface> water_iso = new core::IsoSurface; // x -> z -> y
array<float> water_field;
...
void init_water()
{
	water_geo = new render::Geometry;
	water_mat = g_plus->GetRenderSystem()->LoadMaterial("water.mat");

	water_field.reserve(iso_w * iso_d * iso_h);
}
...
{
	water_iso->Clear(false);
	PolygoniseIsoSurfaceToRenderGeometry(*g_plus->GetRenderSystem(), *water_geo, *water_mat, iso_w - 2, iso_h - 2, iso_d - 2, water_field.c_ptr(), 1, *water_iso, iso_unit);
...
core::sScene scn;
core::Node *sunlight, *backlight;
core::Node *light_cycle_control;
core::Node *cam;
...
core::SimpleGraphicSceneOverlay *gfx, *ui_gfx;

core::RenderableSystem *renderable_system;
...
void draw_game_state_ui()
{
	...
	g_plus->Text2D(200, 600, stringify("%1").arg(int(health)), 90.f, Color::White, "Carton_Six.ttf");
...
//
render::Geometry *green_disk, *red_disk;
...
void init_lighting()
{
	...
	light_cycle_control = new core::Node();
	auto trs = (core::Transform *)light_cycle_control->AddComponent(new core::Transform);
...
#ifdef PACKED
	g_fs->Mount(new io::CFile, "@sys/");
	auto zip_h = g_fs->Open("@sys/data.zip");
	__RASSERT_MSG__(zip_h, "WWTFBBQ: Missing data.zip archive");
	g_fs->Mount(new io::Zip(zip_h));
#else
	g_plus->MountFilePath("d:/gs-users/ggj2017/data");
#endif
...
//	g_plus->AddLight(scn, Matrix4::RotationMatrix(Vector3(0.6, -0.4, 0)), core::Light::Model_Linear, 300);
	...
	gfx = new core::SimpleGraphicSceneOverlay(false);
